...
import { findTenant, findUser, listMembers, listTasks, workspaceCounts } from "@/lib/data";
  ...
  return <DashboardShell active="grid" user={user} tenant={tenant}>
    <div className="page-wrap">
      ...
      <section className="metric-grid">
        <article><div className="metric-icon violet">↗</div><div><span>Total members</span><strong>{counts.memberCount}</strong><small>Active in this workspace</small></div></article>
      ...
      <section className="dashboard-grid">
        <article className="surface team-preview">
          ...
          <div className="overview-task-list">
            {tasks.slice(0, 4).map(task => <div className="overview-task" key={task.id}>
              <span className={`status-dot ${task.status}`}/><div><strong>{task.title}</strong><small>{task.assignee?.name || "Former member"}</small></div>
        ...
        <aside className="surface workspace-card">
          ...
          <div className="workspace-detail"><span>Administrators</span><strong>{counts.adminCount}</strong></div>
      ...
      <section className="surface people-strip">
        ...
        <div>{members.map((member, index) => <div className="person-chip" key={member.id}><span className={`avatar color-${index % 4}`}>{member.name?.[0]?.toUpperCase()}</span><div><strong>{member.name}</strong><small className="capitalize">{member.role}</small></div></div>)}</div>
      </section>
    </div>
  </DashboardShell>;
}
